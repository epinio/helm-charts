...
ensure_epinio() {
  echo "Will now install Epinio helm chart: ${EPINIO_HELM_CHART_VERSION}"
  if ! helm status -n epinio epinio; then
    helm upgrade --install epinio --version $EPINIO_HELM_CHART_VERSION -n epinio --create-namespace epinio/epinio \
